<!-- 案例分类管理页面（树形 CRUD） -->
<template>
  <div class="case-category-page art-full-height">
...
<script setup lang="ts">
  ...
  import CaseCategoryDialog from './modules/case-category-dialog.vue'
  ...
  const loadData = async () => {
    loading.value = true
    try {
      tableData.value = await fetchCaseCategoryList(searchForm)
    } finally {
      loading.value = false
    }
  }
  ...
  const handleSubmit = async (formData: any) => {
    await fetchSaveCaseCategory(formData)
