...
// SysCronDao is the data access object for the table xy_sys_cron.
...
// SysCronColumns defines and stores column names for the table xy_sys_cron.
type SysCronColumns struct {
	Id        string // 任务ID
	GroupId   string // 分组ID
	Title     string // 任务标题
	Name      string // 任务标识（唯一，对应代码注册名）
	Params    string // 任务参数（逗号分隔）
	Pattern   string // Cron表达式
	Policy    string // 策略:1并行,2单例,3单次,4固定次数
	Count     string // 固定次数（policy=4时有效）
	Sort      string // 排序
	Remark    string // 备注
...
// sysCronColumns holds the columns for the table xy_sys_cron.
