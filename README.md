...
│   │   ├── cmdtools/          # Tool command logic (migrate, check-tpl)
...
│   ├── cmd_tools/migrate/     # Migration SQL files + guide
│   ├── manifest/config/       # Runtime config
│   ├── hack/config.yaml       # CLI tool config (build/gen)
│   ├── resource/              # Static assets, code gen templates, SQL
│   ├── main.go                # Server entry (gf run main.go)
│   └── tools.go               # Tools entry (gf run tools.go)
